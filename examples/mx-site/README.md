...
A demo of `@mxlang/translator` — stock `.marko` templates compiled to
...
`@mxlang/translator/bun`'s Bun loader (see `bunfig.toml`).
...
`@mxlang/translator` fails with `Export named 'X' not found in module
