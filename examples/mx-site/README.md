...
A demo of `@markox/translator` — stock `.marko` templates compiled to
...
`@markox/translator/bun`'s Bun loader (see `bunfig.toml`).
...
mapping for *any* file under `packages/translator/`, regardless of which
script is the entry point — so a plain `bun run` of anything that imports
`@markox/translator` fails with `Export named 'X' not found in module
