...
`print()` currently lives at `packages/oracle/src/print.ts` (as `printAst`) because `feat/vite-plugin` had not merged when the retarget landed. On rebase, delete it and import `print` from `@mx/parser`.
