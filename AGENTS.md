...
Current state (round 2): 22 of 30 fixtures pass outright; the other 8 carry a `meta.json` citing a settled decision (S3, S11/decision 47, or a parser limitation recorded in `notes/team-standalone-mx.md`) or a genuinely irreconcilable toolchain difference (Marko drops HTML comments; Marko leaves an element-less template's sole placeholder completely unescaped, unlike a placeholder inside any element). None are unclassified.
