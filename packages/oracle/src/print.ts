 ...
 * shape Vite's `transform` hook accepts.
 */
...
/**
 * Prints a parsed MX AST back as JSX source text plus a source map.
 *
 ...
 * both the native and the Babel backend, so the oracle's `native` backend
 * needs it to have anything to feed the compiler.
 *
 * TEMPORARY LOCATION. `print()` belongs in `@mx/parser` (`src/mx/print.ts`),
 * where the parallel `feat/vite-plugin` branch implements it against the same
 * generator options. This copy exists so the oracle's native backend can be
 * built before that branch merges; on rebase, delete this file and import
 * `print` from `@mx/parser` instead. Deliberately takes an already-parsed
 * `File` rather than source text so the two differ only in where the parse
 * happens, keeping the reconciliation mechanical.
 *
