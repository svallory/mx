...
/**
 * The exact tail `@mxlang/translator`'s `brandRender` (`translate.ts`)
 ...
 * coupled to exactly how `@mxlang/core`/`@mxlang/translator` format a
  ...
  return {
    ...
    transform(code: string, id: string) {
      ...
      if (wrapped === null) {
        ...
        // did not match `@mxlang/translator`'s actual emitted shape (a drift
        ...
        throw new Error(
          `@mxlang/astro: could not wrap ${path} as a page — the compiled ` +
            "module did not match @mxlang/translator's expected branded " +
            "export shape. This file is under src/pages and should be a " +
            "page; if @mxlang/translator's emit format changed, update " +
