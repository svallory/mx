...
//
// Run from anywhere: `bun run packages/translator/scripts/consumer-check.ts`.
...
if (simulateMissingDist) {
  ...
  fail(
    `dist/ is missing at ${distDir} — run \`bun run build\` in packages/translator first. (This is exactly the failure --simulate-missing-dist demonstrates.)`,
  );
}
