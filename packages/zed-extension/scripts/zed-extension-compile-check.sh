...
# packages/tree-sitter-solidmx/scripts/zed-compile-check.sh's rationale (the
...
#
# Usage: scripts/zed-extension-compile-check.sh
...
PKG_REL="packages/zed-extension"
CRATE_NAME="mxlang_zed"

if ! command -v git >/dev/null 2>&1; then
  echo "zed-extension-compile-check.sh: git not found" >&2
  exit 2
fi
if ! command -v cargo >/dev/null 2>&1; then
  echo "zed-extension-compile-check.sh: cargo not found" >&2
  exit 2
fi
if ! rustup target list --installed 2>/dev/null | grep -q '^wasm32-wasip1$'; then
  echo "zed-extension-compile-check.sh: wasm32-wasip1 target not installed; run 'rustup target add wasm32-wasip1'" >&2
  exit 2
fi
...
if [[ ! -f "$PKG_DIR/Cargo.toml" ]]; then
  echo "zed-extension-compile-check.sh: $PKG_DIR/Cargo.toml missing from committed HEAD" >&2
  exit 2
fi
...
set -e
if [[ "$rc" -ne 0 ]]; then
  echo "zed-extension-compile-check.sh: cargo build failed (exit $rc)" >&2
...
if [[ ! -s "$WASM_OUT" ]]; then
  echo "zed-extension-compile-check.sh: $WASM_OUT missing or empty after a reported-successful build" >&2
  exit 1
fi

echo "zed-extension-compile-check.sh: OK — $(basename "$WASM_OUT") compiled from a clean clone of HEAD ($(wc -c < "$WASM_OUT") bytes)"
