...
- To regenerate deliberately (e.g. after a documented pin bump), delete the
  stale file(s) under `__golden__/` and rerun `bun run oracle` or
  `bun run test`. Never let a golden change as a silent side effect of a pin
  bump — call it out in the PR that bumps the pin.
...
treat a clean `skipped` run as the oracle passing.
